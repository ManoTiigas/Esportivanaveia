...
const { MAX_NOTIFICATIONS_PER_USER, serializeNotification } = require('../utils/notifications');

router.get('/', authMiddleware, async (req, res) => {
  try {
    const limit = Math.min(parseInt(req.query.limit, 10) || MAX_NOTIFICATIONS_PER_USER, MAX_NOTIFICATIONS_PER_USER);
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    await ref.delete();
    res.json({ success: true, message: 'Notificacao removida' });
  } catch (err) {
    console.error('Erro ao remover notificacao:', err);
    res.status(500).json({ success: false, message: 'Erro ao remover notificacao' });
  }
});
